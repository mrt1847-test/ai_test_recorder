...
// Side Panel이 열릴 때 recordingData를 로드하고 표시
(function initRecordingData() {
  async function loadRecordingData() {
    try {
      // recordingData 로드 (Background Script에서 저장한 값)
      const result = await chrome.storage.local.get(['recordingData']);
      if (result.recordingData) {
        const data = result.recordingData;
        console.log('[Side Panel] 녹화 데이터 로드:', data);
        
        // testArchitectParams에도 저장 (기존 로직과 호환성)
        ...
        // tcId와 projectId 입력 필드가 있으면 자동 채우기
        const tcIdInput = document.getElementById('tc-id-input');
        const projectIdInput = document.getElementById('project-id-input');
        
        if (data.tcId && tcIdInput && !tcIdInput.value) {
          const tcIdNum = parseInt(data.tcId, 10);
          if (!isNaN(tcIdNum) && tcIdNum > 0) {
            tcIdInput.value = tcIdNum;
            console.log('[Side Panel] TC ID 자동 설정:', tcIdNum);
          }
        }
        ...
        if (data.projectId && projectIdInput) {
          ...
          if (!isNaN(projectIdNum) && projectIdNum > 0) {
            if (!projectIdInput.value || projectIdInput.value === '1') {
              projectIdInput.value = projectIdNum;
              console.log('[Side Panel] Project ID 자동 설정:', projectIdNum);
            }
          }
        }
      }
      
      // 기존 자동 채우기 로직도 실행 (URL에서 파라미터 추출)
      checkAndAutoFillIds();
    } catch (error) {
      console.error('[Side Panel] 녹화 데이터 로드 실패:', error);
      // 오류가 발생해도 기존 자동 채우기 로직은 실행
      checkAndAutoFillIds();
    }
  }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', loadRecordingData);
  } else {
    // 이미 로드된 경우 즉시 실행
    loadRecordingData();
  }
  ...
  function checkAndAutoFillIds() {
    ...
    chrome.storage.local.get(['testArchitectParams'], (result) => {
      if (result.testArchitectParams) {
        ...
        console.log('[Side Panel] 저장된 파라미터 읽기:', params);
      ...
      chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        if (tabs && tabs.length > 0) {
          ...
          if (tab && tab.url) {
            ...
            if (typeof autoFillIdsFromUrl === 'function') {
        ...
        chrome.tabs.query({ active: true }, (allTabs) => {
          if (allTabs && allTabs.length > 0) {
            ...
            if (normalTabs.length > 0) {
              ...
              if (tab && tab.url && typeof autoFillIdsFromUrl === 'function') {
                autoFillIdsFromUrl(tab.url);
              }
            }
          }
    ...
    // Side Panel이 닫힐 때 리스너 제거 (메모리 누수 방지)
